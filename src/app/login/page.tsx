    ...
    try {
      if (isLogin) {
        ...
        const { error } = await supabase.auth.signUp({ 
          ...
          password,
          options: {
            data: { rol: 'cliente' } // Asignamos el rol por defecto
          }
        });
        if (error) throw error;
        setMensaje({ tipo: 'exito', texto: '¡Cuenta creada! Revisa tu correo para verificar (si aplica) o inicia sesión.' });
        setIsLogin(true);
      }
    } catch (error: any) {
      setMensaje({ tipo: 'error', texto: error.message || 'Ocurrió un error.' });
    } finally {
      setLoading(false);
    }
  };
      ...
      <Card className="w-full max-w-md shadow-lg border-zinc-200">
        <CardHeader className="text-center space-y-2">
          ...
          <CardDescription className="text-zinc-500">
            {isLogin ? 'Ingresa tus credenciales para continuar comprando.' : 'Únete a Los Compadres en segundos.'}
          </CardDescription>
        </CardHeader>
        ...
        <form onSubmit={handleAuth}>
          <CardContent className="space-y-4">
            {mensaje && (
              <div className={`p-3 rounded-md text-sm font-medium ${mensaje.tipo === 'error' ? 'bg-red-50 text-red-600' : 'bg-green-50 text-green-600'}`}>
                {mensaje.texto}
              </div>
            )}
